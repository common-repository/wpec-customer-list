...
<?php /* _e('Please review your order', 'wpsc'); */ ?>
<?php if ( wpsc_show_user_login_form() && !is_user_logged_in() ): ?>
	<h3><?php _e('You must sign in or register with us to continue with your purchase', 'wpsc');?></h3>
<?php endif; ?>
...
<table class="checkout_cart">
   <tr class="header">
      ...
      <th><?php /* _e('Price', 'wpsc'); */ _e('Total', 'wpsc'); ?></th>
        <th>&nbsp;</th>
   </tr>
   <?php while (wpsc_have_cart_items()) : wpsc_the_cart_item(); ?>
	 ...
	 <td class="wpsc_product_price wpsc_product_price_<?php echo wpsc_the_cart_item_key(); ?>"><span class="pricedisplay">* <?php $cartitemid=wpsc_cart_item_product_id(); 
		$full_price = get_post_meta( $cartitemid, '_wpsc_price', true );
		$special_price = get_post_meta( $cartitemid, '_wpsc_special_price', true );
		$price = $full_price;
		...
		echo $output = wpsc_currency_display( $price,$args ); ?></span></td>
   ...
   <?php
   if(is_user_logged_in() && wpsc_uses_coupons() && get_option('customer_reward_form_show') == 1){
	global $current_user, $wpdb;
	get_currentuserinfo();
	$couponlist = get_user_meta($current_user->ID, 'wpec-customer-coupons', true);
      if(!empty($couponlist)){
	$coupons_string = implode($couponlist,',');
	//$coupons = $wpdb->get_results($wpdb->prepare());
	/*
		if(count($couponlist)>1)
			$coupons_string = 'IN ('.implode($couponlist,',').')';
		else
			$coupons_string = '= '.$couponlist[0];
	*/
      }
	$coupons = $wpdb->get_results( "SELECT * FROM `" . WPSC_TABLE_COUPON_CODES . "` WHERE id IN (".$coupons_string.")", ARRAY_A ); 

      if(!empty($coupons)){
	    if(wpsc_coupons_error())
	      echo '<tr class="wpsc_coupon_row wpsc_coupon_error_row"><td colspan="6">'.__('Coupon is not valid.', 'wpsc').'</td></tr>';
	    //echo '<tr><td colspan="2">'.__('Select coupon code', 'wpec-customerlist').'</td><td colspan="4">';
	    echo '<tr class="wpsc_coupon_row"><td colspan="6">';
	    echo '<form  method="post" action="'.get_option('shopping_cart_url').'">';
	    echo '<select name="coupon_num" id="coupon_num">';
	    echo '<option value="">'.__('Select coupon code', 'wpec-customerlist').'</option>';
	    //echo '<option value="">--</option>';
	    foreach($coupons as $coupon){
		switch ($coupon['is_percentage']){
			case 0:
			      $percent = wpsc_get_currency_symbol();
			      break;
			case 1:
			      $percent = '%';
			      break;
			case 2:
			      $percent = __('Free Shipping', 'wpsc');
			      break;
			default:
			      $percent = wpsc_get_currency_symbol();
			      break;
		}
		echo '<option value="'.$coupon['coupon_code'].'" '.(($coupon['coupon_code'] == $wpsc_cart->coupons_name) ? "selected='true'" : '').'>'.$coupon['value'].$percent.'</option>';
	    }
	    echo '</select>';
	    echo '<input type="submit" value="'.__('Update', 'wpsc').'" />';
	    echo '</form>';
	    echo '</td></tr>';
      }
    }
   ...
   <?php do_action('wpsc_before_form_of_shopping_cart'); ?>
	...
	<?php if ( wpsc_show_user_login_form() && !is_user_logged_in() ): ?>
		...
		<div class="wpsc_registration_form">
			...
			<h3><a class="signin_toggle" href="<?php echo wp_login_url(); ?>"><?php _e( 'Sign in', 'wpsc' ); ?></a></h3>
			<div class="wpsc_signup_text"><?php _e('If you subscribed as a Daily Deal Hunter, please provide further necessary registration details to continue the purchase.', 'wpec-dd');?>
			</div>
    ...
    <?php if(wpsc_show_user_login_form()):
		...
		<div class="wpsc_registration_form wpsc_right_registration">
			
		    <h3><a class="reg_button" href="<?php echo wp_login_url().'?action=register'; ?>"><?php _e('Join up now', 'wpsc');?></a></h3>

	            <div class="wpsc_signup_text"><?php _e('Signing up is free and easy! please fill out your details your registration will happen automatically as you checkout. Don\'t forget to use your details to login with next time!', 'wpsc');?>
		    </div>
...
<!-- div for make purchase button -->
      <div class='wpsc_make_purchase'>
         <span>
            ...
            <?php endif; ?>
               <input type='hidden' value='complex_checkout' name='wpsc_instant_action' />
	 <?php if ( wpsc_show_user_login_form() && is_user_logged_in() || !wpsc_show_user_login_form() ): ?>
